...
constexpr float ANGLE_MAX = 1.3f; //отскок
...
void Racket::handleEvent(const sf::Event& event) { // гор€чие клавиши
    switch (event.type) {
...
std::function<void(void)> Racket::collideWith(DefenderBlock& block) const { //блоки
    return [](void) -> void { return; };
}

std::function<void(void)> Racket::collideWith(DefenderBonus& bonus) const { // бонусы
...
std::function<void(void)> Racket::collideWith(DefenderBall& ball) const { //м€ч
    ...
    return [=, &ball](void) -> void {
        if (_parent.isStickyBallEnabled()) {
            // ѕровер€ем, касаетс€ ли шарик каретки
